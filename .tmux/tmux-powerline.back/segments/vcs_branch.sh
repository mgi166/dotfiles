...
run_segment() {
	...
	if [ -n "${svn_branch=$(__parse_svn_branch)}" ]; then
		branch="$svn_branch"
	elif [ -n "${git_branch=$(__parse_git_branch)}" ]; then
		branch="$git_branch"
...
__parse_svn_branch() {
	...
	local branch_name=$(echo "${svn_info}" | sed -ne 's#^URL: ##p' | tr -s '/' ' ' | awk -F ' ' '{print $3}')

	local branch=$(echo "${svn_url}" | egrep -o '[^/]+$')
	echo "#[fg=colour${svn_colour}]${branch_symbol} #[fg=colour${TMUX_POWERLINE_CUR_SEGMENT_FG}]${branch_name}: ${branch}"
}
